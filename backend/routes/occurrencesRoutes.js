...
function registerOccurrencesRoutes(context) {
    ...
    app.post('/api/occurrences/import/supabase', (_req, res) => {
        // Ocorrências vivem apenas no WA PRO (SQLite). Import do Supabase desativado — dados migrados.
        return res.status(410).json({ success: false, error: 'Import de ocorrências do Supabase desativado. As ocorrências existem apenas no WA PRO.' });
    });
}
