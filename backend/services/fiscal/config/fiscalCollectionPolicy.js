...
function assessFiscalCollectionNeed(job, data, options = {}) {
    ...
    if (job === 'pme') {
        ...
        const last = parseDate(meta.completedAt || meta.startedAt || meta.requestedAt);
        if (last && last > addMonths(now, -12)) return { shouldCollect: false, requiresConfirmation: true, reason: 'Certificado PME recolhido há menos de 1 ano. Quer recolher novamente?' };
