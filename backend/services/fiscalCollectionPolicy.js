...
function assessFiscalCollectionNeed(job, data, options = {}) {
    ...
    if (job === 'ies') {
        if (isParticular) {
            return { shouldCollect: false, requiresConfirmation: false, reason: 'IES não se aplica a particulares.' };
        }
        if (hasFilingForYear(data.ies, targetYear)) {
            return { shouldCollect: false, requiresConfirmation: true, reason: `IES de ${targetYear} já consta no resumo. Quer avançar com nova recolha?` };
        }
        return { shouldCollect: true, requiresConfirmation: false, reason: `IES de ${targetYear} em falta.` };
    }

    if (job === 'modelo22') {
        const label = isParticular ? 'IRS' : 'Modelo 22';
        if (hasFilingForYear(data.modelo22, targetYear)) {
            return { shouldCollect: false, requiresConfirmation: true, reason: `${label} de ${targetYear} já consta no resumo. Quer avançar com nova recolha?` };
        }
        return { shouldCollect: true, requiresConfirmation: false, reason: `${label} de ${targetYear} em falta.` };
    }
    ...
    if (job === 'certidao_at' || job === 'certidao_ss') {
        ...
        if (validUntil && validUntil > addDays(now, 30)) {
            return { shouldCollect: false, requiresConfirmation: true, reason: `${JOB_LABELS[job]} válida por mais de 1 mês. Quer recolher novamente?` };
        }
    ...
    if (job === 'domicilio_fiscal') {
        if (isEmpresa) {
            return { shouldCollect: false, requiresConfirmation: false, reason: 'Domicílio fiscal está configurado só para particulares.' };
        }
        const last = parseDate(meta.completedAt || meta.startedAt || meta.requestedAt);
        if (last && last > addMonths(now, -6)) {
            return { shouldCollect: false, requiresConfirmation: true, reason: 'Domicílio fiscal recolhido há menos de 6 meses. Quer recolher novamente?' };
        }
    ...
    if (job === 'bportugal') {
        ...
        if (sameMonth(last, now)) {
            return { shouldCollect: false, requiresConfirmation: true, reason: 'Responsabilidades Banco de Portugal já recolhidas este mês. Quer recolher novamente?' };
        }
    ...
    if (job === 'certidao_permanente') {
        ...
        if (validUntil && validUntil > addDays(now, 30)) {
            return { shouldCollect: false, requiresConfirmation: true, reason: 'Certidão Permanente válida por mais de 1 mês. Quer consultar novamente?' };
        }
